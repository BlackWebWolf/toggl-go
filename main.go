...
type DetailedTimeEntry struct {
	...
	Billable        bool       `json:"is_billable"`
}
...
func (session *Session) GetDetailedReport(workspace int, since, until string, user_id int) (DetailedReport, error) {
	params := map[string]string{
		"user_agent":   "jc-toggl",
		"since":        since,
		"until":        until,
		"user_ids":     fmt.Sprintf("%d", user_id),
		"rounding":     "on",
		"workspace_id": fmt.Sprintf("%d", workspace)}
